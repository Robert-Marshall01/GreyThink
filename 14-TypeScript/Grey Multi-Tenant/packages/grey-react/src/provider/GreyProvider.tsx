...
export function GreyProvider({ apiBaseUrl, children }: GreyProviderProps): React.ReactElement {
  ...
  useEffect(() => {
    ...
    const unsubAuth = authController.subscribe((state) => {
    ...
    const unsubUser = userController.subscribe((state) => {
      if (mountedRef.current) {
    ...
    const unsubProjects = projectsController.subscribeToList((state) => {
      if (mountedRef.current) {
        setProjectsData({
