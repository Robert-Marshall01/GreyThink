# Grey Multi‑Tenant — Expo Framework Implementation

This file defines the Expo‑specific rules, folder structure, and file responsibilities for the Grey Multi‑Tenant SDK.
...
Expo code must be idiomatic, mobile‑safe, web‑safe, and free of Node/DOM APIs.
...
- No Node APIs (no fs, path, crypto unless via Expo APIs)
- Optional: Expo SecureStore for persistence (only if explicitly requested)
...
- Expose async actions:
  ...
  - refresh
- Normalize errors.
- Must be Expo‑safe (no DOM, no Node).
...
- Contain no UI logic
- Be Expo‑safe (no DOM, no Node)

============================================================
RUNTIME RULES
============================================================

- Expo has no SSR; ignore SSR‑specific rules.
- Hooks must not access browser APIs.
- Hooks must not access Node APIs.
...
Optional persistence imports (only if requested):
