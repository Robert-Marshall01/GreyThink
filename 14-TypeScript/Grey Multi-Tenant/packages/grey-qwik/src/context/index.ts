/**
 * Grey Qwik - Context Management
 *
 * Provides initialization and context utilities for the Grey SDK.
 * Uses Qwik's context API for dependency injection.
 */
...
/**
 * Grey context ID for Qwik's context system
 */
export const GreyContextId = createContextId<GreyContext>('grey-context');
...
export function useGreyContext(): GreyContext {
  const context = useContext(GreyContextId);
  if (!context) {
    throw new Error(
      '[Grey Qwik] Grey context not found. Wrap your app with <GreyProvider>.'
    );
  }
  return context;
}
 ...
 * import { useClient } from '@grey/qwik';
 *
...
export function createGreyContext(config: GreyConfig): GreyContext {
  ...
  // Query cache for query composables
