...
if [ -x "$HOME/.local/bin/grey-core-api" ]; then
    BIN_PATH="$HOME/.local/bin/grey-core-api"
    CONFIG_DIR="$HOME/.config/grey-multitenant"
    LOG_DIR="$HOME/.local/share/grey-multitenant/logs"
elif [ -x "/usr/local/bin/grey-core-api" ]; then
    BIN_PATH="/usr/local/bin/grey-core-api"
    CONFIG_DIR="/etc/grey-multitenant"
    LOG_DIR="/var/log/grey-multitenant"
elif [ -x "$SCRIPT_DIR/grey-core-api" ]; then
    BIN_PATH="$SCRIPT_DIR/grey-core-api"
    CONFIG_DIR="$SCRIPT_DIR"
    LOG_DIR="$SCRIPT_DIR/logs"
elif [ -x "$SCRIPT_DIR/../bin/grey-core-api" ]; then
    BIN_PATH="$SCRIPT_DIR/../bin/grey-core-api"
...
# Find config file
CONFIG_FILE=""
for path in "$CONFIG_DIR/.env" "$SCRIPT_DIR/.env" "$HOME/.config/grey-multitenant/.env" "/etc/grey-multitenant/.env"; do
    if [ -f "$path" ]; then
        CONFIG_FILE="$path"
        break
    fi
...
# Check systemd PostgreSQL
if [ "$PG_READY" = false ]; then
    if systemctl is-active --quiet postgresql 2>/dev/null; then
        log "INFO" "PostgreSQL running via systemd"
        PG_READY=true
    fi
fi
...
if [ "$PG_READY" = false ]; then
    ...
    if command -v docker &>/dev/null; then
        ...
        if [ "$CONTAINER_EXISTS" = "grey-postgres" ]; then
            ...
            for compose_dir in "$HOME/Grey Multi-Tenant" "/opt/grey-multitenant" "$SCRIPT_DIR/../.."; do
...
if [ "$PG_READY" = false ]; then
    ...
    echo "  1. Install Docker and run: docker compose up -d postgres"
    echo "  2. Install PostgreSQL: sudo apt install postgresql && sudo systemctl start postgresql"
    echo ""
